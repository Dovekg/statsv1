...
@section('content')
    <!-- Advanced login -->
    <form role="form" method="POST" action="{{ url('/analyst/register/' . $token) }}">
        {{ csrf_field() }}
        <div class="panel panel-body login-form">
            <div class="text-center">
                <div class="icon-object border-success text-success"><i class="icon-plus3"></i></div>
                <h5 class="content-group">分析员注册<small class="display-block">必须填写所有内容</small></h5>
            </div>

            <div class="content-divider text-muted form-group"><span>分析员资料</span></div>
            ...
            <div class="form-group has-feedback has-feedback-left">
                <input type="text" name="phone" class="form-control" placeholder="电话号码" value="{{ old('phone') }}">
                <div class="form-control-feedback">
                    <i class="icon-phone-plus2 text-muted"></i>
                </div>
                @if ($errors->has('phone'))
                    <span class="help-block text-danger"><i class="icon-cancel-circle2 position-left"></i>{{ $errors->first('phone') }}</span>
                @endif
            </div>
